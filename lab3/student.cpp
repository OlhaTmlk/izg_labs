...
void pinedaPolygon(const Point *points, const int size, const RGBA &color1, const RGBA &color2)
{
    ...
    // abych měla s čím porovnat hodnoty x,y ve for (dál)
    int minX = points[0].x;
    int maxX = points[0].x;
    int minY = points[0].y;
    int maxY = points[0].y;
    ...
    //  Pro ukladani parametru hranovych funkci muzete vyuzit prichystany vektor parametru hranovych funkci "EdgeParams":
    EdgeParams edgeParams(size); // deklarace vektoru edgeParams

    //  Pro ukladani hodnot hranovych funkci muzete vyuzit prichystany vektor hodnot hranovych funkci "EdgeFncValues":
    EdgeFncValues edgeFncValues(size); // deklarace vektoru edgeFncValues

    for (int i = 0; i < size; i++) {
        int next = i + 1; // index nasledujiciho bodu 
        if (i + 1 == size) //(jsme na konci vektiru) 
            next = 0; 
            //dostaneme smerovy vektor (∆𝑥, ∆𝑦):
        // ℎ = (𝑥1−𝑥0, 𝑦1 − 𝑦0, 0) = (∆𝑥, ∆𝑦, 0)
        edgeParams[i].deltaX = points[next].x - points[i].x; // x souradnice
        edgeParams[i].deltaY = points[next].y - points[i].y; // y souradnice
        //𝐸(𝑥, 𝑦) = (𝒚 − 𝒚𝟎)*∆𝒙 − (𝒙 − 𝒙𝟎)*∆y
    ...
    for (int y = minY; y < maxY; y++) {
        EdgeFncValues hr_hodnoty = edgeFncValues; // kopie hodnot hranovych funkci do pole 'hr_hodnoty'
        for (int x = minX; x < maxX; x++) {
            bool neg_prostor = false; // hodnoty jsou v kladnem prostoru 
            for (int i = 0; i < size; i++) { 
                if (hr_hodnoty[i] < 0) neg_prostor = true; // nejaka hodnota je v zapornem prostoru
            }
            if (!neg_prostor) putPixel(x, y, color1); // vykresleni, pokud jsme v kladnem prostoru

            //aktualizace hodnot Ei(x,y) ---> E(x + 1; y)
            for (int j = 0; j < size; j++) {
                hr_hodnoty[j] -= edgeParams[j].deltaY; // odecteni deltyY pro každý následujicí x (𝑬 (𝒙 + 𝟏, 𝒚) = 𝑬 (𝒙, 𝒚) - ∆𝒚 )
            }
        }
        for (int k = 0; k < size; k++) {
            edgeFncValues[k] += edgeParams[k].deltaX; // pricteni deltyX pro každý následujicí y (𝑬 (𝒙, 𝒚 + 𝟏) = 𝑬(𝒙, 𝒚) + ∆𝒙 )
        }
    }
